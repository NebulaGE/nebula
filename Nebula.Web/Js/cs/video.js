...
function videoViewModel() {
    ...
    self.exerciseViewModel.setAnswerUrl = U + 'CSVideo/SetAnswer';
    self.exerciseViewModel.resetUrl = U + 'CSVideo/StartAgain';
    ...
    //self.exerciseViewModel = function () {
    ...
    function initWrapper(response) {
        ...
        setTimeout(function () {
            ko.utils.arrayForEach(response.tagsNavigation,
                function (v) {
                    if (response.hasLicense || !v.isPayed) {
                        videoUrl = U + 'CSvideo/index?tagId=' + v.id;
                    } else {
                        videoUrl = U + 'payment';
                    }
        ...
        $.each(response.videoNavigation, function (i, v) {
            ...
            self.videoNavigation.push({ id: v.id, name: v.name, tagId: v.tagId, selected: v.selected, num: k });
    ...
    if (id) {
        ...
        $.post(U + 'CSVideo/GetVideo', { tagId: tagId }, function (response) {
            initWrapper(response);
            if (response.video) {
                if (response.video.isExerciseType) {
                    self.exerciseType(true);
    ...
    //    pageLoading(true);
    //    $.post(U + 'CSVideo/StartAgain?Id=' + id,
    //            function(response) {
    //            })
    //        .done(function() {
    ...
    self.resetQuestions = function () {
        ...
        $.post(self.exerciseViewModel.resetUrl, { Id: id },
            function (response) {
                if (response.success) {
                    $.post(U + 'CSVideo/GetVideo', { videoId: id },
                  function (response) {
                      initExercise(response.video);
                  })
              .done(function () {
